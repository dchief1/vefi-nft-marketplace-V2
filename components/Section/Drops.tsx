    ...
    <div className="flex flex-nowrap flex-row justify-start items-center w-full overflow-scroll py-2 md:my-auto my-2 gap-2">
      {_.map(mockData, (data, index) => (
        <div key={index} className="w-[296.78px]">
          <div className="card card-bordered shadow-xl h-full w-[inherit] bg-transparent">
            <figure className="w-full">
              <img src={data.image} alt={data.collectionName} />
            </figure>
            <div className="card-body w-full">
              <div className="flex justify-between items-center w-full">
                <h2 className="card-title text-[#fff]">{data.collectionName}</h2>
                <label
                  className={`rounded-[5px] py-1 px-1 flex justify-center border-[1px] ${
              ...
              <span className="text-[#e21950] font-[400] text-[10px]">{data.artistName}</span>
            </div>
          </div>
        </div>
      ))}
